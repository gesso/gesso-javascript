...
const enqueue = async (fid, input) => {
  ...
  __promiseQueue__[correlationId] = queueObject;

  // Send input and await output.
  const channel = `gesso:function:${fid}:${correlationId}`;
  const unsubscribe = nrp.on(channel, output => {
    const duration = Date.now() - queueObject.startTime;
    ...
    queueObject.resolve(output);
  });

  // Emit function invocation.
  nrp.emit(`gesso:function:${fid}`, {
    correlationId,
    source: fid,
    input
  });
