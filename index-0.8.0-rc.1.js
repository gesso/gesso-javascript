...
const print = (() => {
  const levels = ["info", "debug", "warn", "log", "error"];
  return levels.reduce((printer, level) => {
    printer[level] = message =>
      DISABLE_LOGGING ? {} : console[level](message);
    return printer;
  }, {});
})();
...
const prepare = () => {
  ...
  const fignore = (() => {
    try {
      ...
      print.info(".gessoignore.json not found");
      return;
    }
  })();
  ...
  for (const fdir of fdirs) {
    ...
    try {
      ...
      if (fignore && fignore[fmanifest.name]) {
        print.info("checking .gessoignore");
        print.info(fmanifest);
        // check version
        if (fignore[fmanifest.name] === fmanifest.version) {
          print.info(`${fmanifest.name} version mismatch: ignoring`);
          continue
        } else {
          print.info(`${fmanifest.name} version match: ok`);
        }
      }
...
const emit = async (fid, input) => {
  ...
  const unsubscribe = nrp.on(channel, output => {
    ...
    const hasResponse = unsubscribe(() => {
      print.info(`Unsubscribe ${channel}`);
    });
    print.info(`hasResponse: ${hasResponse}`);
